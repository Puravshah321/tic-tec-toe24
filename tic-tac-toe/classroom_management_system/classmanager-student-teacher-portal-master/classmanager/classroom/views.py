...
from classroom.forms import UserForm, TeacherProfileForm, StudentProfileForm, MarksForm, MessageForm, NoticeForm, AssignmentForm, SubmitForm, TeacherProfileUpdateForm, StudentProfileUpdateForm
from .forms import UserForm,StudentProfileForm,RegistrationForm
import qrcode
...
#     register=False
#     user_form = UserForm  # Initialize the form variable
#     student_profile_form = StudentProfileForm()  # Initialize student_profile_form
...
#             user = user_form.save()  # Save the form data
...
#     else:
#         user_form = RegistrationForm()  # Create a new form instance if GET request

#     return render(request, 'classroom/signup/student_signup.html', {'user_form': user_form,
...
def user_login(request):
    if request.method == 'POST':
        ...
        if user is not None:
            login(request, user)
            return redirect('dashboard')  # redirect to the appropriate view after login
        else:
